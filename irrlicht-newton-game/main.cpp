...
class ScriptManager {
 ...
 private:
  ...
  static void transformCallback(const NewtonBody *body, const dFloat *matrix, int threadIndex) {
    ...
    if (!node) {
      printf("[ERROR] Node not found for body\n");
      return;
    }
  ...
  static void applyForceAndTorqueCallback(const NewtonBody *body, dFloat timestep, int threadIndex) {
    ...
    dVector gravityVec = dVector(0, -1, 0);
    dFloat gravityForce = 9.8f;
    dVector force = gravityVec.Scale(mass * gravityForce);

    NewtonBodySetForce(body, &force.m_x);
  }

  void initPhysics() {
    const int solverPasses = 4;
    const int threads = 4;

    newtonWorld = NewtonCreate();

    NewtonSetSolverModel(newtonWorld, solverPasses);
...
int main() {
  ...
  */
  device->getGUIEnvironment()->addImage(
      driver->getTexture(
          "irrlicht-newton-game/irrlicht-newton-game.runfiles/__main__/irrlicht-newton-game/media/textures/irrlichtlogoalpha2.tga"),
  ...
  while (device->run()) {
    // Get the time since last frame render, in milliseconds
    const u32 now = device->getTimer()->getTime();

    // Calculate the frame rate
    const f32 frameRate = (f32) 1.0f / ((now - then) * 100);
    then = now;
    ...
    scriptMgr->handleFrame(frameRate); // run scripts handling frame being rendered
